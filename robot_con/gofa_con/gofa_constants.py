...
class GoFaConstants:
    ...
    L_HOME_STATE = GoFaState([0, -130, 135, 30, 0, 40, 0])
    L_HOME_POSE = RigidTransform(translation=[0.123, 0.147, 0.124], rotation=[0.06551, 0.84892, -0.11147, 0.51246])

    R_HOME_STATE = GoFaState([0, -130, -135, 30, 0, 40, 0])
    R_HOME_POSE = RigidTransform(translation=[-0.0101, -0.1816, 0.19775],
                                 rotation=[-0.52426, 0.06481, -0.84133, -0.11456])

    R_FORWARD_STATE = GoFaState([9.66, -133.36, -110.18, 34.69, -13.19, 28.85, 28.81])
    R_FORWARD_POSE = RigidTransform(translation=[0.07058, -0.26519, 0.19775],
    ...
    AXIS_ALIGNED_STATES = {
        'inwards': {
            'right': GoFaState([75, -90, 45, 0, -30, 90, -10]),
            'left': GoFaState([-75, -90, 45, 0, 30, 90, 10])
        },
        'forwards': {
            'right': GoFaState([36.42, -117.3, -100.28, 35.59, 50.42, 46.19, 66.02]),
            'left': GoFaState([-36.42, -117.3, 100.28, 35.59, -50.42, 46.19, 113.98])
        },
        'downwards': {
            'right': GoFaState([-60.0, -110.0, 30.0, 35.0, 116.0, 100.0, -90.0]),
            'left': GoFaState([60.0, -110.0, -30.0, 35.0, -116.0, 100.0, -90.0])
        }
    }
    ...
    R_READY_STATE = GoFaState([51.16, -99.4, -36.00, 21.57, -107.19, 84.11, 94.61])
    L_READY_STATE = GoFaState([-51.16, -99.4, 36.00, 21.57, 107.19, 84.11, 94.61])
    ...
    L_KINEMATIC_AVOIDANCE_STATE = GoFaState([-62.62, -20.31, 36.71, 44.91, 78.61, 74.79, -90.07])
    L_EXPERIMENT_DROP_POSE = RigidTransform(translation=[0.30, 0.45, 0.3],
                                            rotation=[0.21353, -0.37697, 0.78321, -0.44596])

    L_RAISED_STATE = GoFaState([5.5, -99.46, 110.11, 20.52, -21.03, 67, -22.31])
    L_RAISED_POSE = RigidTransform(translation=[-0.0073, 0.39902, 0.31828],
                                   rotation=[0.54882, 0.07398, 0.82585, -0.10630])

    L_FORWARD_STATE = GoFaState([-21.71, -142.45, 106.63, 43.82, 31.14, 10.43, -40.67])
    L_FORWARD_POSE = RigidTransform(translation=[0.13885, 0.21543, 0.19774],
    ...
    BOX_CLOSE_SEQUENCE = [
        ...
        ('R', [0, 0.300, 0]),
        ('R', [0.100, 0, 0]),
        ('R', GoFaState([111.30, -65.37, -68.84, -29.93, -50.53, 65.41, -94.59])),
        ('R', GoFaState([147.13, -76.28, -96.95, -58.98, -13.85, 54.95, 54.95])),
        ('R', [0, 0, 0.070]),
        ('R', [-0.074, 0, 0]),
        ('R', [0, -0.012, 0]),
        ('R', [0, 0, -0.040]),
        ('R', GoFaState([132.32, -56.98, -99.36, -8.13, -72.42, 36.44, 135.16])),
        ('R', GoFaState([146.72, -57.09, -98.41, -9.44, -101.74, 69.26, 155.98])),
        ('R', GoFaState([168.36, -64.12, -102.95, -23.60, -118.0, 97.50, 163.97])),
        ('R', GoFaState([168.37, -59.60, -129.70, 2.63, -146.15, 95.29, 199.63])),
        ('R', GoFaState([168.37, -82.24, -148.06, -10.81, -196.49, 115.92, 199.63])),
